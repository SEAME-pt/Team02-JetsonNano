...
LaneDetector::LaneDetector(const std::string& enginePath, std::shared_ptr<zenoh::Session> session)
{
    ...
    float cameraHeight = 0.10f;       // meters
    float cameraPitch = 10.0f;       // degrees down from horizontal
    ...
    float nearDistance = 0.4f;       // meters
    float farDistance = 2.5f;       // meters
...
void LaneDetector::postProcess(cv::Mat& frame)
{
    ...
    cv::Mat bev_result;
    cv::addWeighted(ipm_frame, 0.7, colored_mask, 0.3, 0, bev_result);
