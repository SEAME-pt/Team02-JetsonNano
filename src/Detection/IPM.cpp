...
cv::Mat IPM::applyIPM(const cv::Mat& input)
{
    if (!initialized)
    {
        std::cerr << "IPM not initialized. Call initialize() first."
                  << std::endl;
        return input.clone();
    }
    ...
    cv::Mat result;
    cv::warpPerspective(resizedInput, result, perspectiveMatrix, dstSize,
...
cv::Mat IPM::applyInverseIPM(const cv::Mat& input)
{
    ...
    cv::Mat result;
    cv::warpPerspective(input, result, invPerspectiveMatrix, origSize,
                        cv::INTER_LINEAR);

    return result;
}

void IPM::drawPoints(cv::Mat& image, const std::vector<cv::Point2f>& points,
...
void IPM::calibrateFromCamera(float cameraHeight, float cameraPitch,
                              float horizontalFOV, float verticalFOV,
                              float nearDistance, float farDistance,
                              float laneWidth)
{
    // Calculate source points based on camera parameters
    origPoints = calculateIPMPoints(cameraHeight, cameraPitch, horizontalFOV,
                                    verticalFOV, origSize, nearDistance,
                                    farDistance, laneWidth);
...
IPM::calculateIPMPoints(float cameraHeight,   // in meters
...
)
{
    ...
    std::vector<cv::Point3f> roadPts = {{-laneWidth / 2, 0.0f, farDistance},
...
cv::Mat IPM::transformPoints(const cv::Mat& mask)
{
    ...
    // std::cout << "Found " << points.size() << " non-zero points in mask" <<
    // std::endl;
