                               ...
                               cv::Mat& frame)
{
    ...
    if (!midCurve.empty())
    {
        ...
        // {
        //     // Use higher smoothing (more history weight) when there are
        //     issues float gamma = lanesConvergedIncorrectly ? 0.9 : 0.8; //
        //     Increased from 0.8

        //     // If detected lanes are bad, rely more on history and bias
        //     toward center if (lanesConvergedIncorrectly) {
        //         // Bias toward center on lane convergence issues
        //         float centerBias = 0.2;
        //         midPoint.x = static_cast<int>(gamma * prevMidPoint.x + (1 -
        //         gamma) *
        //                    ((1-centerBias) * midPoint.x + centerBias *
        //                    (width/2)));
        //     } else {
        //         midPoint.x = static_cast<int>(gamma * prevMidPoint.x + (1 -
        //         gamma) * midPoint.x);
        //     }
        //     midPoint.y = static_cast<int>(gamma * prevMidPoint.y + (1 -
        //     gamma) * midPoint.y);
        // }
...
//         for (int y = height; y >= height * 0.33; y -= 10) { // Changed from
//         20 to 10 for denser points
//             int leftX = -1, rightX = -1;
...
//     // First pass: Check if all points might be the same lane based on
//     proximity bool allPointsPotentiallySameLane = false;
...
//             for (size_t j = i + 1; j < std::min(i + 5,
//             filteredPoints.size()); j++) {
//                 totalDistance += std::sqrt(std::pow(filteredPoints[i].x -
//                 filteredPoints[j].x, 2) +
//                                         std::pow(filteredPoints[i].y -
//                                         filteredPoints[j].y, 2));
//                 numPairs++;
//             }
//         }
...
//             if (avgDistance < frame.cols * 0.15) { // Points are close to
//             each other
...
//         if (avgDistToLeft < avgDistToRight && avgDistToLeft < frame.cols *
//         0.15) {
//             // All points are likely from left lane
//             leftPoints = filteredPoints;
//             rightPoints.clear();
//         }
//         else if (avgDistToRight < avgDistToLeft && avgDistToRight <
//         frame.cols * 0.15) {
...
//     if (useHistory && !projectedLeftLane.empty() &&
//     !projectedRightLane.empty()) {
...
//                     // If very close to an existing left point, assign to
//                     left if (pointDist < frame.cols * 0.06) { // 6% of frame
//                     width
...
//                     double pointDist = std::sqrt(std::pow(pt.x - rightPt.x,
//                     2) +
//                                                std::pow(pt.y - rightPt.y,
//                                                2));

//                     // If very close to an existing right point, assign to
//                     right if (pointDist < frame.cols * 0.06) { // 6% of frame
//                     width
...
//             // Only if not already assigned based on proximity to other
//             points if (!assignedToGroup) {
//                 // Original distance ratio assignment
//                 double distanceRatio = minDistLeft / (minDistLeft +
//                 minDistRight);
...
//         if (potentialLeftPoints.size() > 3 && potentialRightPoints.size() >
//         3) {
...
//             // Check if these are truly right lane points by looking at lane
//             continuity bool probablyAllRightLane = true;
...
//                 // If a significant percentage of points are near the right
//                 lane, they're all right lane double percentNearRightLane =
//                 (double)pointsNearRightLane / totalPoints; if
//                 (percentNearRightLane > 0.7) { // If 70% or more points are
//                 near the right lane
//                     rightPoints = potentialRightPoints;
...
//             // // Only use the forced split if we're not confident they're
//             all right lane
...
//             filteredPoints.begin() + leftCount);
//             //     rightPoints.assign(filteredPoints.begin() + leftCount,
//             filteredPoints.end());
//             // }
//         }
...
//                     if (minDist < frame.cols * 0.2) { // More tolerant
//                     threshold
//                         pointsNearLeftLane++;
//                     }
//                 }

//                 double percentNearLeftLane = (double)pointsNearLeftLane /
//                 totalPoints; if (percentNearLeftLane > 0.7) { // 70%
//                 threshold
//                     leftPoints = potentialLeftPoints;
//                     rightPoints.clear();
...
//             // In extreme curves, we might need to start over with simple
//             position leftPoints.clear(); rightPoints.clear();
