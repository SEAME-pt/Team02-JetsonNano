...
void LaneDetectorCV::detect(Mat& frame)
{
    ...
    if (!firstFrame)
    {
            ...
            prevLeftCurve.size() == leftCurve.size())
        {
            ...
            {
                leftCurve[i].x = (int)(alpha * leftCurve[i].x +
                                       (1 - alpha) * prevLeftCurve[i].x);
                leftCurve[i].y = (int)(alpha * leftCurve[i].y +
            ...
            prevRightCurve.size() == rightCurve.size())
        {
            ...
            {
                rightCurve[i].x = (int)(alpha * rightCurve[i].x +
                                        (1 - alpha) * prevRightCurve[i].x);
                rightCurve[i].y = (int)(alpha * rightCurve[i].y +
    ...
    if (!midCurve.empty())
    {
        // Use the bottom-most point of the mid curve (or average multiple
        // points)
        size_t bottom_idx = 0;
