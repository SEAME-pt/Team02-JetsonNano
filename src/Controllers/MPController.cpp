...
ModelPredictiveController::ModelPredictiveController(XboxController* xbox_controller)
{

}

ModelPredictiveController::ModelPredictiveController(const std::string& configFile, XboxController* xbox_controller)
{
    this->x0.setZero();
    ...
    trajectory_subscriber.emplace(session_->declare_subscriber(
        ...
        [this](const zenoh::Sample& sample) {   
            ...
            std::cout << "Received mid coeffs: [" << a << ", " << b << ", " << c << "]" << std::endl;
...
void ModelPredictiveController::run()
{
    while (true)
    {
        ...
        if (sae_level.find("SAE_5") != std::string::npos || sae_level.find("SAE_4") != std::string::npos)
        {
...
Control ModelPredictiveController::solve(const Eigen::Vector4d& x0, const std::vector<double>& traj_coeffs)
{
    ...
    for (size_t k = 0; k <= N_; ++k) {
        ...
        double x = traj_coeffs[0] + traj_coeffs[1]*y + traj_coeffs[2]*y*y + traj_coeffs[3]*y*y*y;
        double dx_dy = traj_coeffs[1] + 2*traj_coeffs[2]*y + 3*traj_coeffs[3]*y*y;
        double psi = std::atan(dx_dy) + M_PI/2.0; // Adjust heading because Y is perpendicular
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        for (size_t k = 0; k < N_; ++k) {
            ...
            grad(2*k+1) += 2 * (R_(1,1) * u_flat(2*k+1) + Q_(2,2) * dx(2) + Q_(0,0) * dx(0) + Q_(1,1) * dx(1));
        }

        Eigen::Vector4d dxN = x_seq[N_] - x_ref[N_];
        grad(2*(N_-1)) += 2 * (Qf_(3,3) * dxN(3) + Qf_(0,0) * dxN(0) + Qf_(1,1) * dxN(1));
        grad(2*(N_-1)+1) += 2 * (Qf_(2,2) * dxN(2) + Qf_(0,0) * dxN(0) + Qf_(1,1) * dxN(1));
        ...
        for (size_t k = 0; k < N_; ++k) {
            u_flat(2*k) = std::max(0.0, std::min(1.0, u_flat(2*k))); // throttle [0, 1]
            u_flat(2*k+1) = std::max(-0.7854, std::min(0.7854, u_flat(2*k+1))); // steering [-45deg, +45deg] in radians
        }
    }
...
Eigen::Vector4d ModelPredictiveController::backwardEuler(const Eigen::Vector4d& x, const Eigen::Vector2d& u) {
...
static std::vector<Eigen::Vector2d> projectCurveToGround(
    const std::vector<double>& coeffs_uv,
    double focal_length, int image_width, int image_height,
    double camera_height, double pitch_rad) {
    ...
    for (int v = 0; v <= image_height; v += image_height / 5) {
        double u = coeffs_uv[0] + coeffs_uv[1]*v + coeffs_uv[2]*v*v + coeffs_uv[3]*v*v*v;
...
static std::vector<double> fitThirdDegreePolynomial(const std::vector<Eigen::Vector2d>& points) {
...
//     auto ground_points = MPController::projectCurveToGround(coeffs_uv, 800.0, 1280, 720, 1.2, 0.1);
