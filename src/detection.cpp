    ...
    while (running)
    {
            ...
            !object_mask.empty())
        {
            ...
            if (!writersInitialized) {
                ipmWriter.open("ipm_output.avi", cv::VideoWriter::fourcc('M','J','P','G'), 20, new_frame.size());
                origWriter.open("orig_output.avi", cv::VideoWriter::fourcc('M','J','P','G'), 20, original_frame.size());
