...
PidCalibrator::PidCalibrator(std::shared_ptr<zenoh::Session> session)
    : session_(session), kp_(0.0f), ki_(0.0f), kd_(0.0f), fixed_delta_time_(0.1f)
{
...
void PidCalibrator::run()
{
    ...
    std::cout << "Enter PID values (kp, ki, kd) or 'exit' to quit:" << std::endl;
    std::string input;
    while (true)
    {
        std::cout << "PID> ";
        ...
        try
        {
            ...
            {
                throw std::invalid_argument("Invalid input format. Use: kp,ki,kd");
            }
